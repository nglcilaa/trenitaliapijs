...
import { getRegion, getDepartures, getTrainIcon, getTrainTypeIcon, stazioniGenova, stazioniSpeciali, stazioniLombardia } from './functions/trainService.js';
...
app.get('/stations/:partenza_id/departures', async (req, res) => {
  ...
  try {
    ...
    const result = await Promise.all(departures.map(async d => {
      const origineStazione = await getTrainOrigin(d.numeroTreno); // Ottieni stazione di provenienza
